...
def run_app():
    if request.method=='GET':
        ...
        if pred==1:
            res='visible'
            #if the link is malicious determine the type of malicious attack that it is!
            for i in range(114):
                url.append(0)
            mal_phish_or_def=typemodel.predict([url])[0]
            if mal_phish_or_def==1:
                typeofattack='phishing'
            elif mal_phish_or_def==2:
                typeofattack='defacement'
            else:
                typeofattack='malware'

        return render_template('index.html',text=text,result=result,typeofattack=typeofattack,res=res,col=col)
